...
const RoundDetails: React.FC = () => {
  ...
  const isCurrentRound =
    currentRound && currentRound.round_id.toString() === roundId;
  ...
  // Show error only if we can't get the specific round data AND takes
  // (auction details is optional for round view)
  if (!specificRound || !takesResponse) {
    return (
  ...
  const roundInfo = isCurrentRound
    ? currentRound
    : specificRound
    ? {
        ...
        transaction_hash: specificRound.transaction_hash,
        is_active: specificRound.is_active,
        total_takes: specificRound.total_takes || takes.length,
        seconds_elapsed: specificRound.round_start
          ? Math.floor(Date.now() / 1000) - specificRound.round_start
          : 0,
        progress_percentage: specificRound.is_active
          ? ((specificRound.total_takes || takes.length) /
              Math.max(1, parseFloat(specificRound.initial_available))) *
            100
          : 100,
      }
    : {
  ...
  // Get token symbols from the specific round data (preferred) or fallback to auction details
  const fromTokenSymbol = 
    specificRound?.from_token_symbol ||
    fromTokens.find(
      (t) => t.address.toLowerCase() === fromTokenAddress.toLowerCase()
    )?.symbol ||
    fromTokenAddress.slice(0, 6) + "…" + fromTokenAddress.slice(-4);
    
  const wantTokenSymbol = 
    specificRound?.want_token_symbol ||
    wantToken?.symbol ||
    "Unknown";
  ...
  const avgPrice =
    takes.length > 0
      ? takes.reduce((sum, sale) => sum + parseFloat(sale.price), 0) /
        takes.length
      : 0;
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500">
                    {formatTimeAgo(
                      new Date(roundInfo.kicked_at).getTime() / 1000
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="font-mono text-gray-200">
                    {formatReadableTokenAmount(roundInfo.initial_available, 4)}{" "}
                    {fromTokenSymbol}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div>
                    {roundInfo?.transaction_hash ? (
                      <StandardTxHashLink
                        txHash={roundInfo.transaction_hash}
                        chainId={parseInt(chainId!)}
                        length={6}
                        className="text-sm"
                      />
              ...
              <div className="space-y-4">
                ...
                {(roundInfo as any)?.current_price && (
                  <div>
                    ...
                    <div className="text-xs text-gray-500">
                      {formatUSD(
                        parseFloat((roundInfo as any).current_price) * 1.5
                      )}
                    </div>
                  </div>
                )}
                ...
                {avgPrice > 0 && (
                  <div>
                    ...
                    <div className="font-mono text-gray-200">
                      {avgPrice.toFixed(6)} {wantTokenSymbol}
                    </div>
                    <div className="text-xs text-gray-500">
                      per {fromTokenSymbol}
                    </div>
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between">
                ...
                <span className="text-gray-300">
                  {auctionDetails?.parameters?.update_interval || "N/A"}s
                </span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-300 font-mono">
                  {auctionDetails?.parameters?.decay_rate
                    ? `${(auctionDetails.parameters.decay_rate * 100).toFixed(
                        3
                      )}%`
                    : "N/A"}
                </span>
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-300 font-mono">
                  ...
                  auctionDetails.parameters.starting_price !== "0"
                    ? auctionDetails.parameters.starting_price
                    : "Dynamic"}
                </span>
              </div>
