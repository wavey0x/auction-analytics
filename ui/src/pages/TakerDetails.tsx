...
const TakerDetails: React.FC = () => {
  ...
  const { data: takesResponse, isLoading: takesLoading } = useQuery({
    ...
    queryFn: () =>
      apiClient.getTakerTakes(address!, {
        page: takesPage,
        limit: takesLimit,
  ...
  const { data: tokenPairsResponse } = useQuery({
    ...
    queryFn: () =>
      apiClient.getTakerTokenPairs(address!, {
        page: tokenPairsPage,
  ...
  const uniqueChains = new Set(
    takerDetails.auction_breakdown.map((ab) => ab.chain_id)
  );
    ...
    <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div
                ...
                onClick={() =>
                  setRankingMode(rankingMode === "takes" ? "volume" : "takes")
                }
            ...
            <div className="flex items-center space-x-2 px-3 py-2 bg-gray-800 rounded-lg">
              ...
              <span className="text-sm text-gray-300">
                ...
                <span className="font-medium">
                  {formatTimeAgo(takerDetails.first_take)}
                </span>
              </span>
            </div>
          )}
          ...
          {takerDetails.last_take && (
            <div className="flex items-center space-x-2 px-3 py-2 bg-gray-800 rounded-lg">
              ...
              <span className="text-sm text-gray-300">
                ...
                <span className="font-medium">
                  {formatTimeAgo(takerDetails.last_take)}
                </span>
              </span>
            </div>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatsCard
          title="Total Volume"
          value={
            takerDetails.total_volume_usd !== null
              ? formatUSD(takerDetails.total_volume_usd)
              : "—"
          }
          icon={DollarSign}
          iconColor="text-green-400"
        ...
        <StatsCard
          ...
          value={
            takerDetails.avg_take_size_usd !== null
              ? formatUSD(takerDetails.avg_take_size_usd)
              : "—"
          }
          icon={Activity}
          ...
          <table className="table">
            <thead className="bg-gray-800">
              <tr>
                ...
                <th className="text-left px-4 py-3">ROUND | TAKE</th>
            ...
            <tbody className="bg-gray-900">
              {takes.map((take) => (
                <tr key={take.take_id} className="group hover:bg-gray-800/50">
                  ...
                  <td className="px-4 py-3">
                    {/* Show compact Round + Take buttons */}
                    <div className="inline-flex items-center">
                      {/* @ts-ignore dynamic type from API */}
                      <RoundTakeDisplay
                        chainId={take.chain_id}
                        auctionAddress={
                          (take as any).auction_address || (take as any).auction
                        }
                        roundId={take.round_id}
                        takeSeq={
                          (take as any).take_seq || (take as any).sequence
                        }
                        size="sm"
                      />
                    </div>
                      ...
                      <div className="text-xs text-gray-400 leading-tight mt-0.5">
                        ...
                        {(take as any).amount_taken
                          ? `${parseFloat(
                            ...
                            })} ${(take as any).from_token_symbol || "tokens"}`
                          : `${parseFloat(take.sold).toLocaleString("en-US", {
                              minimumFractionDigits: 0,
                              maximumFractionDigits: 3,
                            })} tokens`}
                      </div>
                  ...
                  <td className="px-4 py-3">
                    ...
                    {(take as any).amount_taken_usd &&
                    (take as any).amount_paid_usd ? (
                      (() => {
                        const takerPnL =
                          parseFloat((take as any).amount_taken_usd) -
                          parseFloat((take as any).amount_paid_usd);
        ...
        {takesResponse && takesResponse.total_pages > 1 && (
          <div className="mt-4 pt-4 border-t border-gray-800">
            <Pagination
              ...
              onNext={() =>
                setTakesPage((p) => Math.min(takesResponse.total_pages, p + 1))
              }
          ...
          <table className="table">
            ...
            <tbody className="bg-gray-900">
              {tokenPairsResponse?.token_pairs?.map((tokenPair) => (
                <tr
                  key={`${tokenPair.from_token}-${tokenPair.to_token}`}
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span className="font-medium">
                      {tokenPair.volume_usd !== null
                        ? formatUSD(tokenPair.volume_usd)
                        : "—"}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-sm text-gray-400">
                      {tokenPair.last_take_at
                        ? formatTimeAgo(tokenPair.last_take_at)
                        : "—"}
                    </span>
                  </td>
                </tr>
              ))}
              {(!tokenPairsResponse?.token_pairs ||
                tokenPairsResponse.token_pairs.length === 0) && (
                <tr>
                  <td
                    colSpan={6}
