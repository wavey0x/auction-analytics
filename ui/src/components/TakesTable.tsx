...
import TakeCardMobile from "./TakeCardMobile";
...
const TakesTable: React.FC<TakesTableProps> = ({
  ...
  if (takes.length === 0) {
    return (
      <div className="card">
        <h3 className={`text-lg font-semibold ${spacing.header}`}>{title}</h3>
  ...
  return (
    <>
      {title && (
        <div className={`flex items-center justify-between ${spacing.section}`}>
